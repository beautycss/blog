...
([
  ...
  function(module, exports, __webpack_require__) {
    ...
    var render = function render(res) {
      ...
      var imageSize = isLargeScreen ? 'high_size' : 'standard_size';
      ...
      years.forEach(function(year) {
        var monthData = res[year];
        var monthTmpl = '';
        for (var month in monthData) {
          var imageData = monthData[month];
          var liTmpl = "";
          for (var i = 0, len = imageData.length; i < len; i++) {
            var data = imageData[i];
            var minSrc = data.images.thumbnail;
            var src = isLargeScreen ? data.images.high : data.images.standard;
            var type = data.type;
            var target = src + (type === 'video' ? '.mp4' : '.jpg');
            var size = data.images[imageSize];
            var caption = data.caption ? data.caption.text : '';
            minSrc = domain + minSrc;
            src = domain + src;
            ...
            liTmpl += '<figure class="thumb" itemprop="associatedMedia" itemscope="" itemtype="http://schema.org/ImageObject">\
                  <a href="' + src + '" itemprop="contentUrl" data-size="'+size+'" data-type="' + type + '" data-target="' + target + '">\
                    ...
                    <div class="like"><i class="icon-smile"></i>' + data.likes + '</div>\
                  </a>\
                  <figcaption style="display:none" itemprop="caption description">' + caption + '</figcaption>\
              </figure>';
          }
          monthTmpl += '<h2 class="month">' + month + '月</h2>\
          <section class="archives album">\
            <ul class="img-box-ul">' + liTmpl + '</ul>\
          </section>\
        ';
        }
        itemTmpl = itemTmpl + '<h1 class="year">' + year + '年</h1>' + monthTmpl;
        // <h1 class="year">' + data.year + '<em>' + data.month + '月</em></h1>
      });
    ...
    function loadData(success) {
      if (!searchData) {
        ...
        xhr.open('GET', './instagram.json?t=' + +new Date(), true);
